...
int main()
{
    ...
    while(t--)
    {
        int n,m;
        scanf("%d%d",&n,&m);
        max_flow ob(n+m+2);//0 money sink  1-n items   n+1-n+m vouchers  n+m+1 source node
        ll sum=0;
        for(int i=1;i<=n;++i)
        {
            int x;
        ...
        for(int i=1;i<=m;++i)
        {
            ...
            while(k--)
            {
                int x;
                scanf("%d",&x);
                ob.addEdge(n+i,x,INF);
            }
        }
        ll ans=sum-ob.maximum_flow(n+m+1,0);
        printf("%lld\n",ans);
    }
}
